...
const productCopy = {
  fr: { announcement: "Paiement à la livraison · Confirmation par téléphone · Échange facile", back: "← Retour à la boutique", cart: "Voir mon panier", home: "Accueil", save: "Économisez", cod: "Payez seulement à la livraison", codText: "Commandez sans carte. Nous confirmons par téléphone avant l’envoi.", chooseSize: "Choisir l’âge", sizeGuide: "Guide des âges", soldOut: "Épuisé", stock: "en stock", stockLow: "Bientôt épuisé", stockOk: "Disponible", add: "Ajouter au panier", choose: "Choisissez l’âge de l’enfant.", finderTitle: "Trouver la bonne taille", finderLabel: "Stature de l’enfant, debout et pieds nus", finderPlaceholder: "ex. 92", finderResult: "Taille conseillée", finderApply: "Choisir cette taille", finderUnder: "Plus petit que notre plus petite taille : ce modèle risque d’être trop grand.", finderOver: "Plus grand que notre plus grande taille : ce modèle risque d’être trop petit.", finderSoldOut: "Cette taille est épuisée pour le moment.", recommended: "Conseillé", love: "Pourquoi on l’aime", comfort: "Confortable à chaque petit pas", materials: "Matières", care: "Entretien", delivery: "Livraison & paiement", goodChoice: "Bien choisir", sizeAdvice: "Mesurez l’enfant debout et pieds nus comme illustré. Utilisez sa stature en priorité ; l’âge reste indicatif. Entre deux tailles, choisissez la plus grande.", age: "Âge conseillé", weight: "Poids", height: "Stature", discover: "À découvrir aussi", maybe: "Vous aimerez peut-être", addShort: "Ajouter", orderNow: "Commander maintenant", reviews: "avis", deliveryTracked: "Livraison suivie", shippingByWilaya: "Frais calculés selon votre wilaya", exchangeSize: "Échange de taille", exchangeHelp: "Notre équipe vous accompagne", codShort: "Paiement à la livraison", codHelp: "Aucune carte bancaire nécessaire" },
  en: { announcement: "Cash on delivery · Phone confirmation · Easy exchange", back: "← Back to store", cart: "View my cart", home: "Home", save: "Save", cod: "Pay only on delivery", codText: "Order without a card. We confirm by phone before shipping.", chooseSize: "Choose the age", sizeGuide: "Age guide", soldOut: "Sold out", stock: "in stock", stockLow: "Almost gone", stockOk: "Available", add: "Add to cart", choose: "Choose the child’s age.", finderTitle: "Find the right size", finderLabel: "Child’s height, standing barefoot", finderPlaceholder: "e.g. 92", finderResult: "Recommended size", finderApply: "Select this size", finderUnder: "Shorter than our smallest size: this item may be too big.", finderOver: "Taller than our largest size: this item may be too small.", finderSoldOut: "This size is sold out for now.", recommended: "Recommended", love: "Why we love it", comfort: "Comfort for every little step", materials: "Materials", care: "Care", delivery: "Delivery & payment", goodChoice: "Choose well", sizeAdvice: "Measure the child standing barefoot as illustrated. Use height first; age is only a guide. If between two sizes, choose the larger one.", age: "Recommended age", weight: "Weight", height: "Height", discover: "Discover more", maybe: "You may also like", addShort: "Add", orderNow: "Order now", reviews: "reviews", deliveryTracked: "Tracked delivery", shippingByWilaya: "Fee calculated for your wilaya", exchangeSize: "Size exchange", exchangeHelp: "Our team will help you", codShort: "Cash on delivery", codHelp: "No bank card required" },
  ar: { announcement: "الدفع عند الاستلام · تأكيد هاتفي · استبدال سهل", back: "العودة إلى المتجر →", cart: "عرض السلة", home: "الرئيسية", save: "وفروا", cod: "الدفع فقط عند الاستلام", codText: "اطلبوا دون بطاقة. نؤكد الطلب هاتفياً قبل الشحن.", chooseSize: "اختيار المقاس", sizeGuide: "دليل المقاسات", soldOut: "نفد", stock: "متوفر", stockLow: "على وشك النفاد", stockOk: "متوفر", add: "أضف إلى السلة", choose: "اختاروا المقاس.", finderTitle: "اعثر على المقاس المناسب", finderLabel: "طول الطفل وهو واقف وحافي القدمين", finderPlaceholder: "مثال: 92", finderResult: "المقاس المناسب", finderApply: "اختيار هذا المقاس", finderUnder: "أصغر من أصغر مقاس متوفر: قد يكون واسعاً.", finderOver: "أكبر من أكبر مقاس متوفر: قد يكون ضيقاً.", finderSoldOut: "هذا المقاس نفد حالياً.", recommended: "موصى به", love: "لماذا نحبّه", comfort: "راحة مع كل خطوة صغيرة", materials: "الخامات", care: "العناية", delivery: "التوصيل والدفع", goodChoice: "اختيار المقاس", sizeAdvice: "قيسوا طول الطفل وهو واقف وحافي القدمين كما في الصورة. اعتمدوا الطول أولاً، فالعمر إرشادي فقط. بين مقاسين اختاروا الأكبر.", age: "العمر", weight: "الوزن", height: "الطول", discover: "اكتشفوا أيضاً", maybe: "قد يعجبكم أيضاً", addShort: "أضف", orderNow: "اطلب الآن", reviews: "تقييمات", deliveryTracked: "توصيل متابع", shippingByWilaya: "السعر حسب الولاية", exchangeSize: "استبدال المقاس", exchangeHelp: "فريقنا يساعدكم", codShort: "الدفع عند الاستلام", codHelp: "لا حاجة لبطاقة بنكية" },
} as const;
...
export default function ProductDetail({ product, related }: { product: PublicProduct; related: PublicProduct[] }) {
  ...
  // La barre se lit en relatif : pleine pour la taille la mieux fournie du coloris, et elle
  // descend a mesure que la taille se vend. Aucune quantite n'est publiee, et le repere
  // s'ajuste tout seul au reassort au lieu de dependre d'un plafond invente.
  const maxSizeStock = availableSizes.reduce((most, item) => Math.max(most, item.stock), 0);
  const recommendation = useMemo(() => childHeight.trim() ? recommendSize(availableSizes, Number(childHeight)) : null, [availableSizes, childHeight]);
  const recommendedSize = recommendation ? availableSizes.find((item) => item.label === recommendation.label) : undefined;
  ...
  function addToCart(destination: "cart" | "checkout" = "cart") {
    ...
    const addition: CartItem = { productId: product.id, slug: product.slug, name: display.name, image: product.colorImages[selectedColor] || product.images[0] || "", size, sizeLabel: localizedAgeLabel({ label: size, age: selectedSize?.age }, locale), color: selectedColor || undefined, quantity, unitPriceCents: product.priceCents };
    const found = cart.find((item) => item.productId === addition.productId && item.size === addition.size && item.color === addition.color);
    if (found) found.quantity = Math.min(availableStock, found.quantity + addition.quantity);
    else cart.push(addition);
    localStorage.setItem(CART_KEY, JSON.stringify(cart));
    trackCommerce("AddToCart", { content_ids: [contentId(product.slug)], content_type: "product", content_name: display.name, content_category: product.category, contents: [{ id: contentId(product.slug), quantity, item_price: product.priceCents / 100 }], value: product.priceCents * quantity / 100, currency: "DZD", num_items: quantity });
  ...
  return <div className="product-page" dir={dir}>
    ...
    <div className="mobile-buy">{(promise.delay || promise.exchange) && <p className="mobile-buy-promise">{[promise.delay, promise.exchange].filter(Boolean).join(" · ")}</p>}<div><small>{size ? localizedAgeLabel(availableSizes.find((item) => item.label === size) || { label: size }, locale) : display.name}</small><strong>{money(offerTotal)}</strong></div><button disabled={outOfStock} onClick={() => addToCart("checkout")}>{outOfStock ? outOfStockLabel : copy.orderNow}</button></div>
