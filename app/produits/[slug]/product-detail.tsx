  ...
  return <div className="product-page" dir={dir}>
    ...
    <main>
      ...
      <section className="product-detail-grid">
        <div className="gallery"><div className="gallery-main"><Image src={activeImage} alt={display.name} fill priority sizes="(max-width: 900px) 100vw, 55vw" unoptimized={activeImage.startsWith("/api/media/")} />{product.images.length > 1 && <span className="gallery-count">{Math.max(1, product.images.indexOf(activeImage) + 1)} / {product.images.length}</span>}</div>{product.images.length > 1 && <div className="thumbnails">{product.images.map((image) => <button type="button" aria-label={`${display.name} ${product.images.indexOf(image) + 1}`} key={image} className={image === activeImage ? "active" : ""} onClick={() => setActiveImage(image)}><Image src={image} alt="" fill sizes="90px" unoptimized={image.startsWith("/api/media/")} /></button>)}</div>}</div>
        <div className="product-panel">{outOfStock ? <span className="badge standalone badge-out-of-stock">{outOfStockLabel}</span> : product.badge && <span className="badge standalone">{product.badge}</span>}<span className="product-category">{product.category}</span><h1>{display.name}</h1>{reviewCount > 0 && <a className="rating-summary" href="#avis">{reviewAverage > 0 && <><span>{"★".repeat(Math.round(reviewAverage))}{"☆".repeat(5 - Math.round(reviewAverage))}</span><strong>{reviewAverage.toFixed(1)}</strong></>}<u>{reviewCount} {copy.reviews}</u></a>}<p className="product-lead">{display.shortDescription}</p><div className="detail-price"><strong>{money(product.priceCents)}</strong>{product.compareAtCents && <del>{money(product.compareAtCents)}</del>}{product.compareAtCents && <span>{copy.save} {money(product.compareAtCents - product.priceCents)}</span>}</div>
          ...
          <div className="product-reassurance"><p><b>🚚 {copy.deliveryTracked}</b><span>{copy.shippingByWilaya}</span></p><p><b>↔ {copy.exchangeSize}</b><span>{copy.exchangeHelp}</span></p><p><b>✓ {copy.codShort}</b><span>{copy.codHelp}</span></p></div>
        </div>
      </section>
    ...
    <div className="mobile-buy"><div><small>{size ? localizedAgeLabel(availableSizes.find((item) => item.label === size) || { label: size }, locale) : display.name}</small><strong>{money(product.priceCents * quantity)}</strong></div><button disabled={outOfStock} onClick={() => addToCart("checkout")}>{outOfStock ? outOfStockLabel : copy.orderNow}</button></div>
