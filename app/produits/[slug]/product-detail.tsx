...
export default function ProductDetail({ product, related }: { product: PublicProduct; related: PublicProduct[] }) {
  ...
  return <div className="product-page" dir={dir}>
    ...
    <main>
      ...
      {related.length > 0 && <section className="related"><div className="section-heading"><div><span className="eyebrow">{copy.discover}</span><h2>{copy.maybe}</h2></div></div><div className="product-grid compact">{related.map((item) => { const image = item.colorImages[item.colors[0] || item.color] || item.images[0] || "/images/soft-days.jpg"; const itemTranslation = locale === "fr" ? undefined : item.translations[locale]; const itemName = itemTranslation?.name || item.name; return <article className="product-card" key={item.id}><Link href={`/produits/${item.slug}`} className="product-image"><Image src={image} alt={itemName} fill sizes="33vw" unoptimized={image.startsWith("/api/media/")} /></Link><div className="product-info"><span className="product-category">{item.category}</span><Link href={`/produits/${item.slug}`}><h3>{itemName}</h3></Link><strong>{money(item.priceCents)}</strong></div></article>; })}</div></section>}
    </main>
    <div className="mobile-buy"><div><small>{display.name}</small><strong>{money(product.priceCents)}</strong></div><button onClick={addToCart}>{copy.addShort}</button></div>
