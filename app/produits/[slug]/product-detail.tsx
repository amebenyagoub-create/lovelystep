...
import { multiBuySubtotal, priceMultiBuyItems } from "@/lib/multi-buy";
  ...
  return <div className="product-page" dir={dir}>
    ...
    <div className="mobile-buy multi-buy-sticky" aria-label={offerCopy.aria}>
      <div className="mobile-buy-offers">
        ...
        <button type="button" aria-pressed={duoMode} className={duoMode ? "selected" : ""} disabled={duoProducts.length === 0} onClick={() => { setQuantity(1); setDuoOpen(true); }}><span>{offerCopy.badge}</span><small>{duoMode && duoName ? duoName : offerCopy.two}</small><strong>{money(duoTotal)}</strong></button>
      </div>
