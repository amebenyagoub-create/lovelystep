...
function OrdersTable({ orders, onStatus, onDispatch, zrExpressReady, busy }: { orders: Order[]; onStatus: (id: number, status: OrderStatus) => void; onDispatch: (order: Order) => void; zrExpressReady: boolean; busy: boolean }) {
  ...
  return <div className="orders-table"><div className="order-row order-head"><span>N°</span><span>Client</span><span>Articles</span><span>Total</span><span>Date</span><span>Statut et livraison</span></div>{orders.map((order) => {
    ...
    return <div className="order-row" key={order.id}><strong>{order.orderNumber}</strong><span><b>{order.customerName}</b><small>{order.phone}<br />{order.commune || order.city} · {order.wilayaName || order.city}<br />{order.deliveryType === "office" ? "Bureau" : "Domicile"}</small></span><span>{order.items.reduce((sum, item) => sum + item.quantity, 0)} article(s)<small>{order.items.map((item) => `${item.name} (${item.size}${item.color ? ` · ${item.color}` : ""})`).join(", ")}</small></span><strong>{money(order.totalCents)}<small>Livraison {money(order.shippingCents)}</small></strong><span>{new Date(order.createdAt).toLocaleDateString("fr-FR")}</span><div className="order-status-cell"><select disabled={busy} value={order.status} onChange={(event) => onStatus(order.id, event.target.value as OrderStatus)}>{Object.entries(orderLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select>{order.deliverySyncStatus === "sent" ? <><span className="status published">Envoyée à ZR</span><small className="zr-parcel-id">ID colis : {order.deliveryExternalId}</small></> : order.deliverySyncStatus === "pending" ? <span className="status draft">Envoi ZR en cours…</span> : <>{order.deliverySyncStatus === "failed" && <><span className="status failed">Échec ZR Express</span><small className="delivery-sync-error">{order.deliverySyncError}</small></>}{canDispatch ? <button type="button" className="zr-send-button" disabled={busy || !zrExpressReady} onClick={() => onDispatch(order)}>{order.deliverySyncStatus === "failed" ? "Réessayer l’envoi ZR" : "Envoyer à ZR Express"}</button> : <small className="zr-help">Confirmez la commande avant l’envoi.</small>}</>}</div></div>;
  })}</div>;
}
