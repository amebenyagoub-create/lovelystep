...
import type { DeliveryIntegration, DeliveryRate, ImportJob, LocalizedText, Order, OrderStatus, Product, ProductSize, ProductTestimonial, ProductVariant, StoreSettings } from "@/lib/types";
import { frenchAgeLabel } from "@/lib/product-size";

type AdminData = { admin: { email: string }; csrfToken: string; stats: { products: number; published: number; newOrders: number; orders: number; deliveredRevenueCents: number; grossProfitCents: number; visitors30d: number; repeatBuyerRate: number; inventoryUnits: number }; meta: { pixelConfigured: boolean; insightsConfigured: boolean }; products: Product[]; orders: Order[]; imports: ImportJob[]; storeSettings: StoreSettings; deliveryRates: DeliveryRate[]; deliveryIntegration: DeliveryIntegration };
type Tab = "overview" | "orders" | "products" | "import" | "store" | "delivery";
...
export default function AdminDashboard() {
  ...
  return <div className="admin-shell">
    ...
    <main className="admin-main"><header><div><span className="admin-kicker">Lovely Step · Administration</span><h1>{tab === "overview" ? "Bonjour 👋" : tab === "orders" ? "Commandes" : tab === "products" ? "Catalogue produits" : tab === "import" ? "Import produit par IA" : tab === "store" ? "Façade de la boutique" : "Livraison"}</h1></div><span className="secure-pill">● Session sécurisée</span></header>{error && <div className="admin-alert error">{error}<button onClick={() => setError("")}>×</button></div>}{notice && <div className="admin-alert success">{notice}<button onClick={() => setNotice("")}>×</button></div>}
      ...
      {tab === "delivery" && <DeliveryEditor rates={data.deliveryRates} integration={data.deliveryIntegration} busy={busy} onSaveRates={async (rates) => { const value = await jsonRequest("/api/admin/delivery", { method: "POST", body: JSON.stringify({ rates }) }); if (value) setNotice("Tarifs de livraison enregistrés."); }} onSaveIntegration={async (integration) => { const value = await jsonRequest("/api/admin/delivery-integration", { method: "POST", body: JSON.stringify(integration) }); if (value) setNotice("Connecteur de livraison enregistré."); }} />}
    </main>
    {editing && <ProductEditor product={editing === "new" ? null : editing} busy={busy} csrfToken={data.csrfToken} onError={setError} onClose={() => setEditing(null)} onSave={async (body) => { const value = await jsonRequest("/api/admin/products", { method: "POST", body: JSON.stringify(body) }); if (value) { setEditing(null); setNotice("Produit enregistré."); } }} />}
  </div>;
}
...
function DeliveryEditor({ rates, integration, busy, onSaveRates, onSaveIntegration }: { rates: DeliveryRate[]; integration: DeliveryIntegration; busy: boolean; onSaveRates: (rates: DeliveryRate[]) => Promise<void>; onSaveIntegration: (integration: DeliveryIntegration) => Promise<void> }) {
  const [values, setValues] = useState(rates);
  const [connector, setConnector] = useState(integration);
  ...
  return <div className="delivery-admin-layout"><form className="admin-card" onSubmit={(event) => { event.preventDefault(); void onSaveRates(values); }}><div className="card-title"><div><h2>Tarifs par wilaya</h2><p>Montants en DZD, séparés pour domicile et bureau.</p></div><button className="admin-primary" disabled={busy}>Enregistrer les tarifs</button></div><input className="admin-search" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Rechercher une wilaya…" /><div className="delivery-rate-table"><div className="delivery-rate-row head"><span>Wilaya</span><span>Domicile</span><span>Bureau</span><span>Active</span></div>{filtered.map((rate) => <div className="delivery-rate-row" key={rate.wilayaCode}><strong>{rate.wilayaCode} · {rate.wilayaNameFr}<small>{rate.wilayaNameAr}</small></strong><label><input aria-label={`Domicile ${rate.wilayaNameFr}`} type="number" min="0" step="1" value={rate.homeCents / 100} onChange={(event) => update(rate.wilayaCode, { homeCents: Math.round(Math.max(0, Number(event.target.value) || 0) * 100) })} /> DZD</label><label><input aria-label={`Bureau ${rate.wilayaNameFr}`} type="number" min="0" step="1" value={rate.officeCents / 100} onChange={(event) => update(rate.wilayaCode, { officeCents: Math.round(Math.max(0, Number(event.target.value) || 0) * 100) })} /> DZD</label><input aria-label={`Activer ${rate.wilayaNameFr}`} type="checkbox" checked={rate.active} onChange={(event) => update(rate.wilayaCode, { active: event.target.checked })} /></div>)}</div></form><form className="admin-card delivery-connector" onSubmit={(event) => { event.preventDefault(); void onSaveIntegration(connector); }}><div className="card-title"><div><h2>API société de livraison</h2><p>Connecteur générique sécurisé. Adaptez les champs aux documents du transporteur.</p></div></div><label className="toggle-field"><input type="checkbox" checked={connector.enabled} onChange={(event) => setConnector({ ...connector, enabled: event.target.checked })} /> Activer l’envoi automatique des commandes</label><label>Nom de la société<input value={connector.providerName} onChange={(event) => setConnector({ ...connector, providerName: event.target.value })} placeholder="Ex. Yalidine" /></label><label>URL de base HTTPS<input type="url" value={connector.baseUrl} onChange={(event) => setConnector({ ...connector, baseUrl: event.target.value })} placeholder="https://api.transporteur.dz" /></label><label>Endpoint de création<input value={connector.createShipmentPath} onChange={(event) => setConnector({ ...connector, createShipmentPath: event.target.value })} placeholder="/shipments" /></label><label>Nom de la variable secrète<input value={connector.apiTokenEnv} onChange={(event) => setConnector({ ...connector, apiTokenEnv: event.target.value.toUpperCase() })} pattern="[A-Z][A-Z0-9_]{2,79}" /></label><div className="import-warning"><strong>La clé API n’est jamais stockée dans la base</strong><p>Ajoutez-la dans le fichier d’environnement du serveur avec le nom indiqué ci-dessus. Le format de données pourra être ajusté dès que vous aurez choisi la société.</p></div><button className="admin-primary" disabled={busy}>Enregistrer le connecteur</button></form></div>;
}

function OrdersTable({ orders, onStatus, busy }: { orders: Order[]; onStatus: (id: number, status: OrderStatus) => void; busy: boolean }) {
  if (!orders.length) return <div className="empty-admin">Aucune commande pour le moment.</div>;
  return <div className="orders-table"><div className="order-row order-head"><span>N°</span><span>Client</span><span>Articles</span><span>Total</span><span>Date</span><span>Statut</span></div>{orders.map((order) => <div className="order-row" key={order.id}><strong>{order.orderNumber}</strong><span><b>{order.customerName}</b><small>{order.phone}<br />{order.commune || order.city} · {order.wilayaName || order.city}<br />{order.deliveryType === "office" ? "Bureau" : "Domicile"}</small></span><span>{order.items.reduce((sum, item) => sum + item.quantity, 0)} article(s)<small>{order.items.map((item) => `${item.name} (${item.size}${item.color ? ` · ${item.color}` : ""})`).join(", ")}</small></span><strong>{money(order.totalCents)}<small>Livraison {money(order.shippingCents)}</small></strong><span>{new Date(order.createdAt).toLocaleDateString("fr-FR")}</span><select disabled={busy} value={order.status} onChange={(event) => onStatus(order.id, event.target.value as OrderStatus)}>{Object.entries(orderLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>)}</div>;
}
