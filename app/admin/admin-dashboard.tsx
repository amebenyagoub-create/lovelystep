...
type AdminData = { admin: { email: string }; csrfToken: string; stats: { products: number; published: number; newOrders: number; orders: number; deliveredRevenueCents: number; grossProfitCents: number; visitors30d: number; repeatBuyerRate: number; inventoryUnits: number }; meta: { pixelConfigured: boolean; insightsConfigured: boolean }; zrExpress: { apiKeyConfigured: boolean; tenantConfigured: boolean; ready: boolean }; products: Product[]; orders: Order[]; imports: ImportJob[]; storeSettings: StoreSettings; deliveryRates: DeliveryRate[]; deliveryIntegration: DeliveryIntegration };
type Tab = "overview" | "analytics" | "meta" | "orders" | "products" | "import" | "store" | "delivery";
...
export default function AdminDashboard() {
  ...
  const navigation: [Tab, string, string][] = [["overview", "Vue d’ensemble", "⌂"], ["analytics", "Rentabilité", "◫"], ["meta", "Meta", "◎"], ["orders", "Commandes", "▤"], ["products", "Produits", "◇"], ["import", "Import IA", "↧"], ["store", "Façade boutique", "✦"], ["delivery", "Livraison", "▣"]];
...
function DeliveryEditor({ rates, integration, zrExpress, busy, onSyncZrExpress, onSaveRates, onSaveIntegration }: { rates: DeliveryRate[]; integration: DeliveryIntegration; zrExpress: AdminData["zrExpress"]; busy: boolean; onSyncZrExpress: () => Promise<DeliveryRate[] | null>; onSaveRates: (rates: DeliveryRate[]) => Promise<void>; onSaveIntegration: (integration: DeliveryIntegration) => Promise<void> }) {
  ...
  return <div className="delivery-admin-layout">
    ...
    <form className="admin-card delivery-connector" onSubmit={(event) => { event.preventDefault(); void onSaveIntegration(connector); }}>
      <div className="card-title"><div><h2>Envoi des commandes</h2><p>Connecteur serveur pour transmettre les commandes confirmées au transporteur.</p></div></div>
      <label className="toggle-field"><input type="checkbox" checked={connector.enabled} onChange={(event) => setConnector({ ...connector, enabled: event.target.checked })} /> Activer l’envoi automatique des commandes</label>
      <label>Nom de la société<input value={connector.providerName} onChange={(event) => setConnector({ ...connector, providerName: event.target.value })} placeholder="Ex. ZR Express" /></label>
      <label>URL de base HTTPS<input type="url" value={connector.baseUrl} onChange={(event) => setConnector({ ...connector, baseUrl: event.target.value })} placeholder="https://api.zrexpress.app" /></label>
      <label>Endpoint de création<input value={connector.createShipmentPath} onChange={(event) => setConnector({ ...connector, createShipmentPath: event.target.value })} placeholder="/api/v1/parcels" /></label>
      <label>Nom de la variable secrète<input value={connector.apiTokenEnv} onChange={(event) => setConnector({ ...connector, apiTokenEnv: event.target.value.toUpperCase() })} pattern="[A-Z][A-Z0-9_]{2,79}" /></label>
      <div className="import-warning"><strong>La clé API n’est jamais stockée dans la base</strong><p>Les tarifs ZR Express utilisent les variables sécurisées du serveur. N’activez l’envoi automatique des commandes qu’après son test dédié.</p></div>
      <button className="admin-primary" disabled={busy}>Enregistrer le connecteur</button>
    </form>
  </div>;
}
