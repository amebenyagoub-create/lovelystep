...
export async function POST(request: Request) {
  ...
  if (!loginAllowed(email, ip)) return NextResponse.json({ error: "Trop de tentatives. Réessayez dans 15 minutes." }, { status: 429 });
  const admin = verifyAdminCredentials(email, password);
  recordLoginAttempt(email, ip, Boolean(admin));
  if (!admin) return NextResponse.json({ error: "E-mail ou mot de passe incorrect." }, { status: 401 });
  await createSession(admin.id);
  audit(admin.id, "admin.login");
