...
export default function Storefront({ products, settings, wilayas, deliveryRates, freeShippingThresholdCents }: { products: PublicProduct[]; settings: StoreSettings; wilayas: AlgeriaWilaya[]; deliveryRates: DeliveryRate[]; freeShippingThresholdCents: number }) {
  ...
  return <div className="store-shell" style={style} dir={dir}>
    ...
    <footer><Image src="/brand/lovelystep-logo.png" alt="Lovely Step" width={120} height={120} /><p>Tiny Steps, Big Love</p><small>© {new Date().getFullYear()} Lovely Step. {t("rights")}</small><nav className="legal-links"><Link href="/confidentialite">{t("privacy")}</Link><Link href="/suppression-donnees">{t("dataDeletion")}</Link></nav><Link href="/admin/login" className="admin-link">Administration</Link></footer>
