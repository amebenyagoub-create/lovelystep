import { marketingAllowed, readConsentCookie } from "./meta/consent";
import { cleanCustomData, randomEventId, type MetaCustomData, type MetaStandardEvent } from "./meta/events";

declare global {
  interface Window {
    fbq?: (...args: unknown[]) => void;
    _lovelyStepMetaPixel?: string;
    _lovelyStepPendingMetaEvents?: Array<{
      eventName: MetaStandardEvent;
      parameters: MetaCustomData;
      eventId: string;
    }>;
  }
}

/**
 * Fires a browser Pixel event.
 *
 * `eventId` must be supplied for any event that is ALSO sent server-side (Purchase), using the
 * exact id the server used; otherwise Meta counts the conversion twice. Browser-only events get
 * a random id so a future server counterpart can still deduplicate.
 *
 * Returns silently when consent is absent. Events fired before the Pixel is ready wait in a
 * short-lived browser queue; tracking never throws into a checkout or render path.
 */
export function trackMeta(eventName: MetaStandardEvent, parameters: MetaCustomData = {}, eventId?: string): void {
  if (typeof window === "undefined") return;
  if (!marketingAllowed(readConsentCookie())) return;
  const pendingEvent = { eventName, parameters: cleanCustomData(parameters), eventId: eventId ?? randomEventId() };
  if (typeof window.fbq !== "function") {
    (window._lovelyStepPendingMetaEvents ??= []).push(pendingEvent);
    return;
  }
  try {
    window.fbq("track", pendingEvent.eventName, pendingEvent.parameters, { eventID: pendingEvent.eventId });
  } catch {
    // A tracking failure must never break the surrounding user action.
  }
}

export function flushPendingMetaEvents(): void {
  const pending = window._lovelyStepPendingMetaEvents ?? [];
  window._lovelyStepPendingMetaEvents = [];
  for (const event of pending) trackMeta(event.eventName, event.parameters, event.eventId);
}
