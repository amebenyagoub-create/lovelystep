...
export async function updateOrderDetails(id: number, input: OrderEditInput, adminId: number | null): Promise<OrderEditResult> {
  ...
  try {
    ...
    // voir avec l'adresse : on l'efface et l'expedition le resout de nouveau elle-meme.
    const keepHub = input.deliveryType === "office" && String(input.wilayaCode).padStart(2, "0") === before.wilayaCode;
    const hubId = keepHub ? before.deliveryHubId : null;
    const hubName = keepHub ? before.deliveryHubName : null;
    ...
    const updated = await client.query(
      `UPDATE orders SET customer_name=$1, phone=$2, wilaya_code=$3, wilaya_name=$4, commune=$5, city=$5,
         ...
         delivery_hub_id=$13, delivery_hub_name=$14,
